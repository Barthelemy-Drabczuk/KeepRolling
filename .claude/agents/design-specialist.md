...
implementation (that's the main thread's). You own `ARCHITECTURE.md` and
...
1. **Locate it in the system.** Read `ARCHITECTURE.md` (create it, with
...
2. **Define the contract.** Write down the interface the implementation
   must satisfy to make the failing test pass — function signature,
   request/response shape, or data model — concretely enough that two
   different implementers would produce interchangeable code. This is
...
4. **Update `ARCHITECTURE.md`.** Add or amend the component description
