...
description: Use to check project health at any time — start of a session, before planning new work, or whenever asked "what's the status" — not tied to any single requirement's lifecycle. Reads TASKS.md and reports which in-flight items have gone stale, current velocity, and schedule risk against any stated milestones. Use proactively at the start of a session, not just when asked for a status update.
...
only read `TASKS.md` and report on the *time* dimension
...
1. **Staleness.** Read `TASKS.md`. For every item not `done`, compare
...
2. **Velocity.** Count items marked `done` and their completion
   ...
   points.
3. **Schedule risk.** Only if `TASKS.md` or `BUSINESS.md` states a
...
4. **Report back** to the main thread: stale items (with how long, and
   ...
   `TASKS.md` row that's healthy and on schedule.

Never write to `TASKS.md` or any other project file — you are
